...
for arg in "$@"; do
    ...
    if [[ "$target" = /* ]]; then
        target_input_abs="$target"
    else
        target_input_abs="$PWD/$target"
    fi
    target_abs="$(realpath -m -- "$target_input_abs")"

    if [[ -L "$target_input_abs" ]]; then
        echo "atomic_write: target path must not be a symlink: '$target'" >&2
        exit 1
    fi

    if [[ -d "$target_input_abs" ]]; then
        echo "atomic_write: target path is an existing directory: '$target'" >&2
        exit 1
    fi

    if [[ -e "$target_input_abs" && ! -f "$target_input_abs" ]]; then
        echo "atomic_write: target path must be a regular file when it already exists: '$target'" >&2
        exit 1
    fi

    # Existing parent components must not be symlinks.
    probe="$target_input_abs"
    while [[ "$probe" != "/" ]]; do
        probe="$(dirname -- "$probe")"
        if [[ -e "$probe" || -L "$probe" ]]; then
            if [[ -L "$probe" ]]; then
                echo "atomic_write: existing parent component is a symlink: '$probe'" >&2
                exit 1
            fi
        fi
    done
    ...
    if [[ "$target_abs" == "$(realpath -m -- "$source")" ]]; then
        echo "atomic_write: source and target resolve to the same path: '$target'" >&2
        exit 1
    fi

    if [[ -n "${SEEN_TARGETS[$target_abs]+x}" ]]; then
        echo "atomic_write: duplicate target path in one invocation: '$target'" >&2
        exit 1
    fi
    SEEN_TARGETS["$target_abs"]=1

    if ! target_on_txn_fs "$target_abs" "$TXN_FS_DEV"; then
