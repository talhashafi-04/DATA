...
def _decode_primitive_list(words, start, element_size, count):
    ...
    for index in range(count):
        ...
        item = raw[begin : begin + byte_width]
        if len(item) != byte_width:
            raise DecodeError("primitive list runs past the end of the segment")
        values.append(int.from_bytes(item, "little", signed=False))
    return values
...
def _decode_text(words, pointer_index, pointer):
    ...
    if not raw.endswith(b"\x00"):
        raise DecodeError("text field is not nul terminated")
    return raw[:-1].decode("utf-8")


def _decode_point_inline(words, element_start, data_words, pointer_words):
    if data_words < 2 or pointer_words != 0:
        raise DecodeError("Point elements must have two data words and no pointer words")
...
def _decode_segment_inline(words, element_start, data_words, pointer_words):
    if data_words != 0 or pointer_words < 2:
        raise DecodeError("Segment elements must have two pointer fields")
    points_pointer_index = element_start
    label_pointer_index = element_start + 1
...
def _decode_composite_list(words, pointer_index, pointer, element_type):
    ...
    element_count = (tag >> 2) & 0x3FFFFFFF
...
def _decode_pointer(words, pointer_index):
    ...
    if kind == KIND_LIST:
        element_size, _count = _list_layout(pointer)
        if element_size == LIST_POINTER:
            return _decode_pointer_list(words, pointer_index, pointer)
        return _decode_primitive_list(words, _target_index(pointer_index, pointer), element_size, _count)
...
def _decode_message_with_event(message, index, message_id, byte_start, byte_end):
...
def decode_file_with_metrics(path, max_workers=1, message_ids=None):
    if max_workers < 1:
        raise DecodeError("max_workers must be at least one")
    ...
    if max_workers == 1:
        decoded = []
        events = []
        for index, (_, start_offset, end_offset, message) in enumerate(records):
            message_id = message_ids[index] if message_ids else f"message-{index:02d}"
            _, payload, event = _decode_message_with_event(message, index, message_id, start_offset, end_offset)
    ...
    diagnostics = {
        ...
        "max_points_in_message": max((event["point_count"] for event in events), default=0),
